...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; gap: 16px; }
        .card-title { font-size: 1.25rem; font-weight: 700; }

        .search-box { flex: 1; position: relative; }
        .search-box input { width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #E2E8F0; font-family: inherit; outline: none; }
        ...
        .pagination a { padding: 8px 12px; border-radius: 8px; border: 1px solid #E2E8F0; text-decoration: none; color: var(--text); font-size: 13px; }
    </style>
</head>
<body>
    ...
    <div class="card">
        <div class="card-header">
            <form action="{{ route('admin.registrations') }}" method="GET" class="search-box">
                <input type="text" name="search" placeholder="Cari nama, email, atau kode..." value="{{ request('search') }}">
            </form>
            <form action="{{ route('admin.registrations') }}" method="GET">
                <select name="status" onchange="this.form.submit()" style="padding: 12px; border-radius: 12px; border: 1px solid #E2E8F0;">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-family: monospace; font-weight: 700; color: var(--primary);">{{ $reg->registration_code }}</td>
                    <td style="font-weight:600;">{{ $reg->full_name }}</td>
                    <td style="font-size: 12px;">{{ $reg->id_number ?? '-' }}</td>
                    <td style="font-size: 12px; max-width: 150px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $reg->address ?? '-' }}</td>
                    <td>{{ $reg->phone }}</td>
                    <td>{{ $reg->email }}</td>
                    ...
                    <td>
                        @if($reg->ticket)
                        <a href="{{ route('ticket.show', $reg->ticket->token) }}" target="_blank" style="color: var(--primary); text-decoration: none; font-weight: 700;">👁️ Tiket</a>
                        @endif
                    </td>
                </tr>
                @endforeach
